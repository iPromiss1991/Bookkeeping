...
#define QuysUserProtocolURL @""

#define QuysPrivacyURL @""


#pragma mark - 事件回调

typedef void(^QuysTableCellEventClickBlock)(NSIndexPath *indexPath);//!<  tableview cell 点击事件

typedef void(^QuysBtnEventClickBlock)(BOOL choose);//!<  按钮点击事件


typedef void(^QuysTextFieldShouldChangeCharactersInRangeBlock)(BOOL shouldChange);//!<  文本框编辑事件
typedef void(^QuysCallbackBlock)(NSString *txtString);//!<  文本回调事件


typedef void(^QuysLoginFinishBlock)(BOOL loginSuccessed);//!<  登陆完成回调事件
